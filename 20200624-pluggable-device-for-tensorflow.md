...
* Set 1: `PluggableDevice` related classes 
   * class `PluggableDevice`:  a class represents a set of new third-party devices, its device_type attribute (counter part of DEVICE_GPU, DEVICE_CPU) can be a new string registered from plugin or TensorFlow proper adds a string prefix "PLUGGABLE_" to the registed device name(front-end) got from `SE_InitializePlugin` as the device_type attribute, depends on StreamExecutor C API and kernel C API. For the second option(adding "PLUGGABLE_" prefix), Kernel C API can also add the "PLUGGABLE_" prefix to the registered device type so this type can be transparently to the plugin authors.For example, plugin authors provide "GPU" name through `SE_InitializePlugin` and register the kernels to the "GPU" name, and TensorFlow Proper takes the "GPU" as the front-end device name and makes the "PLUGGABLE_GPU" as the device type attribute (counter part of DEVICE_GPU, DEVICE_CPU). 
