...
* **Device type**  
   ...
   ```
* **Subdevice type** 
   Subdevice type is user visible. Users can query the subdevice type of the device if they want to know whether the GPU device is "NVIDIA_GPU", "INTEL_GPU" or "AMD_GPU", through [list_physical_devices](https://www.tensorflow.org/api_docs/python/tf/config/list_physical_devices). 
   This RFC propose that plugged gpu device will override the default gpu device in tensorflow proper. So when users invoke `list_physical_devices`, they can only see the pllugged gpu device. If users want to use default GPU device(CUDA), they need to manually uninstall the plugin.
   For example, in the case of two GPUs in the same system, e.g. NVIDIA GPU + X GPU and installing the X GPU plugin, only X GPU is visible.
    ```
